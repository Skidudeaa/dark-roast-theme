...
"""
Dark Roast: Blue Mountain — Terminal.app Profile Generator
Version: 5.1.0 Companion

WHAT THIS SCRIPT DOES
---------------------
Generates "Dark Roast Blue Mountain.terminal" — an Apple Terminal.app color
...
- Python 3 — use the SYSTEM python3, not a venv or Homebrew python, because
  PyObjC is only available on the system installation:
    /usr/bin/python3 generate-blue-mountain-profile.py

  DO NOT use:
    python3 generate-blue-mountain-profile.py   (if python3 points to Homebrew)
    ~/.pyenv/shims/python3 ...
...
2. A file named "Dark Roast Blue Mountain.terminal" will be created in the
   current directory (same folder as this script).

3. Double-click "Dark Roast Blue Mountain.terminal" in Finder, OR:
     open "Dark Roast Blue Mountain.terminal"
...
4. To activate permanently:
   Terminal.app → Settings → Profiles → select "Dark Roast Blue Mountain"
...
Chrome:
  ...
  foreground    #e5ecf3   crema     — primary text (16.54:1 contrast on void)
  cursor        #00b5b5   teal      — kinetic cursor, matches editor caret
  cursor text   #000b1d   void      — text color inside cursor block
  selection     #052544   espresso  — warm polished surface (v4: was `grain`)
  bold          #e5ecf3   crema     — same as foreground (bold = same weight)

ANSI Normal (indices 0–7):
  0  black       #001228   obsidian  — dark floor (not as deep as void)
  1  red         #f86572   scarlet   — error / critical
  2  green       #60a563   sage      — warm fern (terminal extension)
  3  yellow      #ef912d   amber     — warning / accent
  4  blue        #49accf   slate     — dusty steel (terminal extension)
  5  magenta     #b181e5   mauve     — dried lavender (terminal extension)
  6  cyan        #00b5b5   teal      — live data flow
  7  white       #c3cfdc   bone      — reduced-contrast light text

ANSI Bright (indices 8–15):
  8  bright black    #8797a9   crater-deep — comment gray (v4: was `crater`)
  9  bright red      #f55e6c   scarlet+  — scarlet companion-bright
  10 bright green    #6dba70   sage+     — sage companion-bright
  11 bright yellow   #ebaa1f   gold      — success / stable severity
  12 bright blue     #49c1ea   slate+    — slate companion-bright
  13 bright magenta  #bd87f6   mauve+    — mauve companion-bright
  14 bright cyan     #00caca   teal+     — teal brightened (visibly lighter than #00b5b5)
...
PALETTE = {
    # Chrome
    "background":     "#000b1d",   # void
    "foreground":     "#e5ecf3",   # crema
    "cursor":         "#00b5b5",   # teal
    "cursor_text":    "#000b1d",   # void (inverted on cursor block)
    "selection":      "#052544",   # espresso (Terminal.app uses opaque selection bg)
    "bold_text":      "#e5ecf3",   # crema (bold = same color, different weight)

    # ANSI Normal 0–7
    "ansi_black":     "#001228",   # obsidian
    "ansi_red":       "#f86572",   # scarlet
    "ansi_green":     "#60a563",   # sage
    "ansi_yellow":    "#ef912d",   # amber
    "ansi_blue":      "#49accf",   # slate
    "ansi_magenta":   "#b181e5",   # mauve
    "ansi_cyan":      "#00b5b5",   # teal
    "ansi_white":     "#c3cfdc",   # bone

    # ANSI Bright 8–15
    "ansi_bright_black":   "#8797a9",  # crater-deep
    "ansi_bright_red":     "#f55e6c",  # scarlet lightened
    "ansi_bright_green":   "#6dba70",  # sage lightened
    "ansi_bright_yellow":  "#ebaa1f",  # gold
    "ansi_bright_blue":    "#49c1ea",  # slate lightened
    "ansi_bright_magenta": "#bd87f6",  # mauve lightened
    "ansi_bright_cyan":    "#00caca",  # teal brightened (lighter than #00b5b5)
    "ansi_bright_white":   "#e5ecf3",  # crema
}
...
def build_profile():
    ...
    profile = {
        ...
        # we disable this so bold uses TextBoldColor (#e5ecf3) not the
        # bright ANSI variant.
        "UseBrightBold":          False,
...
def main():
    ...
    output_path = os.path.join(output_dir, "Dark Roast Blue Mountain.terminal")

    print("Dark Roast: Blue Mountain — Terminal.app Profile Generator")
    print("=" * 60)
    ...
    print("     select 'Dark Roast Blue Mountain' → click 'Default'.")
    print()
