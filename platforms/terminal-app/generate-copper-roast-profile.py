...
"""
Dark Roast: Copper Roast — Terminal.app Profile Generator
Version: 5.1.0 Companion

WHAT THIS SCRIPT DOES
---------------------
Generates "Dark Roast Copper Roast.terminal" — an Apple Terminal.app color
...
- Python 3 — use the SYSTEM python3, not a venv or Homebrew python, because
  PyObjC is only available on the system installation:
    /usr/bin/python3 generate-copper-roast-profile.py

  DO NOT use:
    python3 generate-copper-roast-profile.py   (if python3 points to Homebrew)
    ~/.pyenv/shims/python3 ...
...
2. A file named "Dark Roast Copper Roast.terminal" will be created in the
   current directory (same folder as this script).

3. Double-click "Dark Roast Copper Roast.terminal" in Finder, OR:
     open "Dark Roast Copper Roast.terminal"
...
4. To activate permanently:
   Terminal.app → Settings → Profiles → select "Dark Roast Copper Roast"
...
Chrome:
  ...
  foreground    #FFF9F4   crema     — primary text (14.09:1 contrast on void)
  cursor        #24C6B6   teal      — kinetic cursor, matches editor caret
  cursor text   #34251C   void      — text color inside cursor block
  selection     #47352A   espresso  — warm polished surface (v4: was `grain`)
  bold          #FFF9F4   crema     — same as foreground (bold = same weight)

ANSI Normal (indices 0–7):
  0  black       #3A2A20   obsidian  — dark floor (not as deep as void)
  1  red         #FF7D70   scarlet   — error / critical
  2  green       #7DC45F   sage      — warm fern (terminal extension)
  3  yellow      #F5A33A   amber     — warning / accent
  4  blue        #5DB4E8   slate     — dusty steel (terminal extension)
  5  magenta     #DB86D0   mauve     — dried lavender (terminal extension)
  6  cyan        #24C6B6   teal      — live data flow
  7  white       #F3E7DD   bone      — reduced-contrast light text

ANSI Bright (indices 8–15):
  8  bright black    #C4A78E   crater-deep — comment gray (v4: was `crater`)
  9  bright red      #FF9184   scarlet+  — scarlet companion-bright
  10 bright green    #A3DD7B   sage+     — sage companion-bright
  11 bright yellow   #E4B20B   gold      — success / stable severity
  12 bright blue     #68D0F8   slate+    — slate companion-bright
  13 bright magenta  #EC94E0   mauve+    — mauve companion-bright
  14 bright cyan     #5FE0D1   teal+     — teal brightened (visibly lighter than #24C6B6)
...
PALETTE = {
    # Chrome
    "background":     "#34251C",   # void
    "foreground":     "#FFF9F4",   # crema
    "cursor":         "#24C6B6",   # teal
    "cursor_text":    "#34251C",   # void (inverted on cursor block)
    "selection":      "#47352A",   # espresso (Terminal.app uses opaque selection bg)
    "bold_text":      "#FFF9F4",   # crema (bold = same color, different weight)

    # ANSI Normal 0–7
    "ansi_black":     "#3A2A20",   # obsidian
    "ansi_red":       "#FF7D70",   # scarlet
    "ansi_green":     "#7DC45F",   # sage
    "ansi_yellow":    "#F5A33A",   # amber
    "ansi_blue":      "#5DB4E8",   # slate
    "ansi_magenta":   "#DB86D0",   # mauve
    "ansi_cyan":      "#24C6B6",   # teal
    "ansi_white":     "#F3E7DD",   # bone

    # ANSI Bright 8–15
    "ansi_bright_black":   "#C4A78E",  # crater-deep
    "ansi_bright_red":     "#FF9184",  # scarlet lightened
    "ansi_bright_green":   "#A3DD7B",  # sage lightened
    "ansi_bright_yellow":  "#E4B20B",  # gold
    "ansi_bright_blue":    "#68D0F8",  # slate lightened
    "ansi_bright_magenta": "#EC94E0",  # mauve lightened
    "ansi_bright_cyan":    "#5FE0D1",  # teal brightened (lighter than #24C6B6)
    "ansi_bright_white":   "#FFF9F4",  # crema
}
...
def build_profile():
    ...
    profile = {
        ...
        # we disable this so bold uses TextBoldColor (#FFF9F4) not the
        # bright ANSI variant.
        "UseBrightBold":          False,
...
def main():
    ...
    output_path = os.path.join(output_dir, "Dark Roast Copper Roast.terminal")

    print("Dark Roast: Copper Roast — Terminal.app Profile Generator")
    print("=" * 60)
    ...
    print("     select 'Dark Roast Copper Roast' → click 'Default'.")
    print()
