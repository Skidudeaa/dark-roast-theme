...
"""
Dark Roast: Velvet Noir — Terminal.app Profile Generator
Version: 5.1.0 Companion

WHAT THIS SCRIPT DOES
---------------------
Generates "Dark Roast Velvet Noir.terminal" — an Apple Terminal.app color
...
- Python 3 — use the SYSTEM python3, not a venv or Homebrew python, because
  PyObjC is only available on the system installation:
    /usr/bin/python3 generate-velvet-noir-profile.py

  DO NOT use:
    python3 generate-velvet-noir-profile.py   (if python3 points to Homebrew)
    ~/.pyenv/shims/python3 ...
...
2. A file named "Dark Roast Velvet Noir.terminal" will be created in the
   current directory (same folder as this script).

3. Double-click "Dark Roast Velvet Noir.terminal" in Finder, OR:
     open "Dark Roast Velvet Noir.terminal"
...
4. To activate permanently:
   Terminal.app → Settings → Profiles → select "Dark Roast Velvet Noir"
...
Chrome:
  ...
  foreground    #F6E4DA   crema     — primary text (16.25:1 contrast on void)
  cursor        #36AFAB   teal      — kinetic cursor, matches editor caret
  cursor text   #14030C   void      — text color inside cursor block
  selection     #3A1528   espresso  — warm polished surface (v4: was `grain`)
  bold          #F6E4DA   crema     — same as foreground (bold = same weight)

ANSI Normal (indices 0–7):
  0  black       #1F0514   obsidian  — dark floor (not as deep as void)
  1  red         #D4445E   scarlet   — error / critical
  2  green       #51A460   sage      — warm fern (terminal extension)
  3  yellow      #E28233   amber     — warning / accent
  4  blue        #6486B9   slate     — dusty steel (terminal extension)
  5  magenta     #C85BA0   mauve     — dried lavender (terminal extension)
  6  cyan        #36AFAB   teal      — live data flow
  7  white       #E3CAC0   bone      — reduced-contrast light text

ANSI Bright (indices 8–15):
  8  bright black    #A88594   crater-deep — comment gray (v4: was `crater`)
  9  bright red      #EC6175   scarlet+  — scarlet companion-bright
  10 bright green    #70BB7C   sage+     — sage companion-bright
  11 bright yellow   #E8A72D   gold      — success / stable severity
  12 bright blue     #7B9ED2   slate+    — slate companion-bright
  13 bright magenta  #E076B7   mauve+    — mauve companion-bright
  14 bright cyan     #59CAC5   teal+     — teal brightened (visibly lighter than #36AFAB)
...
PALETTE = {
    # Chrome
    "background":     "#14030C",   # void
    "foreground":     "#F6E4DA",   # crema
    "cursor":         "#36AFAB",   # teal
    "cursor_text":    "#14030C",   # void (inverted on cursor block)
    "selection":      "#3A1528",   # espresso (Terminal.app uses opaque selection bg)
    "bold_text":      "#F6E4DA",   # crema (bold = same color, different weight)

    # ANSI Normal 0–7
    "ansi_black":     "#1F0514",   # obsidian
    "ansi_red":       "#D4445E",   # scarlet
    "ansi_green":     "#51A460",   # sage
    "ansi_yellow":    "#E28233",   # amber
    "ansi_blue":      "#6486B9",   # slate
    "ansi_magenta":   "#C85BA0",   # mauve
    "ansi_cyan":      "#36AFAB",   # teal
    "ansi_white":     "#E3CAC0",   # bone

    # ANSI Bright 8–15
    "ansi_bright_black":   "#A88594",  # crater-deep
    "ansi_bright_red":     "#EC6175",  # scarlet lightened
    "ansi_bright_green":   "#70BB7C",  # sage lightened
    "ansi_bright_yellow":  "#E8A72D",  # gold
    "ansi_bright_blue":    "#7B9ED2",  # slate lightened
    "ansi_bright_magenta": "#E076B7",  # mauve lightened
    "ansi_bright_cyan":    "#59CAC5",  # teal brightened (lighter than #36AFAB)
    "ansi_bright_white":   "#F6E4DA",  # crema
}
...
def build_profile():
    ...
    profile = {
        ...
        # we disable this so bold uses TextBoldColor (#F6E4DA) not the
        # bright ANSI variant.
        "UseBrightBold":          False,
...
def main():
    ...
    output_path = os.path.join(output_dir, "Dark Roast Velvet Noir.terminal")

    print("Dark Roast: Velvet Noir — Terminal.app Profile Generator")
    print("=" * 60)
    ...
    print("     select 'Dark Roast Velvet Noir' → click 'Default'.")
    print()
