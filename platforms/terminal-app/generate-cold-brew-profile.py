...
"""
Dark Roast: Cold Brew — Terminal.app Profile Generator
Version: 5.1.0 Companion

WHAT THIS SCRIPT DOES
---------------------
Generates "Dark Roast Cold Brew.terminal" — an Apple Terminal.app color
...
- Python 3 — use the SYSTEM python3, not a venv or Homebrew python, because
  PyObjC is only available on the system installation:
    /usr/bin/python3 generate-cold-brew-profile.py

  DO NOT use:
    python3 generate-cold-brew-profile.py   (if python3 points to Homebrew)
    ~/.pyenv/shims/python3 ...
...
2. A file named "Dark Roast Cold Brew.terminal" will be created in the
   current directory (same folder as this script).

3. Double-click "Dark Roast Cold Brew.terminal" in Finder, OR:
     open "Dark Roast Cold Brew.terminal"
...
4. To activate permanently:
   Terminal.app → Settings → Profiles → select "Dark Roast Cold Brew"
...
--------------------
Chrome:
  background    #E2DDD7   void      — warm-dark deepest layer
  foreground    #000000   crema     — primary text (15.56:1 contrast on void)
  cursor        #004444   teal      — kinetic cursor, matches editor caret
  cursor text   #E2DDD7   void      — text color inside cursor block
  selection     #C2B5A7   espresso  — warm polished surface (v4: was `grain`)
  bold          #000000   crema     — same as foreground (bold = same weight)

ANSI Normal (indices 0–7):
  0  black       #D8CFC7   obsidian  — dark floor (not as deep as void)
  1  red         #940015   scarlet   — error / critical
  2  green       #0F3E02   sage      — warm fern (terminal extension)
  3  yellow      #492B00   amber     — warning / accent
  4  blue        #003F60   slate     — dusty steel (terminal extension)
  5  magenta     #472077   mauve     — dried lavender (terminal extension)
  6  cyan        #004444   teal      — live data flow
  7  white       #100701   bone      — reduced-contrast light text

ANSI Bright (indices 8–15):
  8  bright black    #7D6F5F   crater-deep — comment gray (v4: was `crater`)
...
PALETTE = {
    # Chrome
    "background":     "#E2DDD7",   # void
    "foreground":     "#000000",   # crema
    "cursor":         "#004444",   # teal
    "cursor_text":    "#E2DDD7",   # void (inverted on cursor block)
    "selection":      "#C2B5A7",   # espresso (Terminal.app uses opaque selection bg)
    "bold_text":      "#000000",   # crema (bold = same color, different weight)

    # ANSI Normal 0–7
    "ansi_black":     "#D8CFC7",   # obsidian
    "ansi_red":       "#940015",   # scarlet
    "ansi_green":     "#0F3E02",   # sage
    "ansi_yellow":    "#492B00",   # amber
    "ansi_blue":      "#003F60",   # slate
    "ansi_magenta":   "#472077",   # mauve
    "ansi_cyan":      "#004444",   # teal
    "ansi_white":     "#100701",   # bone

    # ANSI Bright 8–15
    "ansi_bright_black":   "#7D6F5F",  # crater-deep
...
def main():
    ...
    output_path = os.path.join(output_dir, "Dark Roast Cold Brew.terminal")

    print("Dark Roast: Cold Brew — Terminal.app Profile Generator")
    print("=" * 60)
    ...
    print("     select 'Dark Roast Cold Brew' → click 'Default'.")
    print()
